    ...
    refreshTask(refreshTask)
{
    ...
    serviceEnabledListItem->getClickEvent()->subscribe([this, serviceEnabledListItem](View* view)
    {
        ...
        brls::Logger::info("新服务状态 = %d", enabled);
    ...
    this->skinTempCell = new StatusCell("外壳", formatTemp(context.temps[SysClkThermalSensor_Skin]));
...
void StatusTab::updateWarningForProfile(SysClkProfile profile, bool animated)
{
    switch (profile)
    {
        case SysClkProfile_Handheld:
            ...
            this->warningLabel->setText("\uE140  最大GPU频率为 " + formatFreq(SYSCLK_GPU_HANDHELD_MAX_HZ) + " 因为您处于手持模式.");
            break;
        case SysClkProfile_HandheldChargingUSB:
            if (this->warningLabel->isHidden())
                this->warningLabel->show([](){});

            this->warningLabel->setText("\uE140  最大GPU频率为 " + formatFreq(SYSCLK_GPU_UNOFFICIAL_CHARGER_MAX_HZ) + " 因为你使用的是非官方充电器.");
            break;
        default:
            if (!this->warningLabel->isHidden())
