...
int main(int argc, char* argv[])
{
    ...
    if (!sysclkIpcRunning())
    {
        brls::Logger::error("sys-clk 未运行");
        brls::Application::crash("sys-clk 似乎没有运行，请检查它是否已正确安装并启用.");
    }
    ...
    else if (R_FAILED(sysclkIpcInitialize()) || R_FAILED(sysclkIpcGetAPIVersion(&apiVersion)))
    {
        ...
        brls::Logger::error("sys-clk IPC API版本不匹配 (预期: %u; 实际: %u)", SYSCLK_IPC_API_VERSION, apiVersion);
        brls::Application::crash("超频管理器与当前运行的sysclk特斯拉模块不兼容，请检查是否已正确安装最新版本（或重新启动？）.");
    }
    else
    {
        // Set version string
        ...
        {
            brls::Logger::error("无法获取sys-clk版本");
            brls::Application::setCommonFooter("[未知]");
        }
