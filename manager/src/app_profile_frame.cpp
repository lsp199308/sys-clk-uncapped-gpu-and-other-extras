...
void AppProfileFrame::addFreqs(brls::List* list, SysClkProfile profile)
{
    ...
    cpuListItem->getValueSelectedEvent()->subscribe([this, profile](int result) {
        ...
        brls::Logger::debug("正在将模块 %d 和配置文件 %d 的频率缓存到 %" PRIu32, SysClkModule_CPU, profile, this->profiles.mhzMap[profile][SysClkModule_CPU]);
    });
    ...
    gpuListItem->getValueSelectedEvent()->subscribe([this, profile](int result) {
        ...
        brls::Logger::debug("将模块 %d 和配置文件 %d 的频率缓存到 %" PRIu32, SysClkModule_GPU, profile, this->profiles.mhzMap[profile][SysClkModule_GPU]);
    });
    ...
    memListItem->getValueSelectedEvent()->subscribe([this, profile](int result) {
        ...
        brls::Logger::debug("将模块 %d 和配置文件 %d 的频率缓存到 %" PRIu32, SysClkModule_MEM, profile, this->profiles.mhzMap[profile][SysClkModule_MEM]);
    });
